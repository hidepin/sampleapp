pipeline {
    ...
    stages {
        ...
        stage('Maven build') {
            ...
            post {
                 success {
                    mattermostSend prext: "@admin @jenkins hogehoge" , color: 'good', message: "BUILD_ID=${env.BUILD_ID} BUILD_DISPLAY_NAME=${env.BUILD_DISPLAY_NAME} BUILD_TAG=${BUILD_TAG} JOB_URL=${JOB_URL} JOB_NAME=${env.JOB_NAME} - BUILD_NUMBER=${env.BUILD_NUMBER} :green_heart: Success after ${currentBuild.durationString} (<${env.BUILD_URL}|Open>) \n BRANCH_NAME=${env.BRANCH_NAME} <br> CHANGE_ID=${env.CHANGE_ID}"
